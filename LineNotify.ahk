...
;
;---------------------------------------------------
LinePush(method, url, oData := "", oHeaders := "") {
	
	FormatTime,LinePushTime,yyyy MM dd HH mm ss,yyyy-MM-dd HH:mm:ss
	data := { "message": "[" LinePushTime "]" oData}	
...
;
;---------------------------------------------------
LinePushImg(method, url, oData := "", oHeaders := "") {
	global title
	pToken:=Gdip_Startup() 
	pBitmapHayStack1:=Gdip_BitmapFromhwnd(hwnd := WinExist(title))
	Gdip_SaveBitmapToFile(pBitmapHayStack1,A_scriptDir "\temp.jpg",8) ; debug save ImageFile
	Gdip_DisposeImage(pBitmapHayStack1)
	Gdip_Shutdown(pToken)
			
	FormatTime,LinePushTime,yyyy MM dd HH mm ss,yyyy-MM-dd HH:mm:ss
	data := { "message": "[" LinePushTime "]" oData,"imageFile":["temp.jpg"]}	
